...
from PokerRL.game._.tree._.MCStrategyFiller import MCStrategyFiller as StrategyFiller
from PokerRL.game._.tree._.MCValueFiller import MCValueFiller as ValueFiller
...
class MCPublicTree:
    ...
    def __init__(self,
        ...
        as the root.
        """
        self._env_bldr = env_bldr # <PokerRL.game.wrappers.HistoryEnvBuilder object at 0x7f0e73b57780>
        self._is_debugging = is_debugging # False
        self._stack_size = stack_size # [13, 13]
        self._sample_method = sample_method
        # sample_methods = ['eps_greedy', 'vr']
        self._epsilon = 0.1
        ...
        # self._env <PokerRL.game.games.StandardLeduc object at 0x7fc46006f828>
        _ar = self._env.get_args()
        # _ar <PokerRL.game.poker_env_args.LimitPokerEnvArgs object at 0x7fc58005a048>
        _ar.RETURN_PRE_TRANSITION_STATE_IN_INFO = True
        self._env.set_args(_ar)
        self._env.reset()

        self._n_seats = self._env_bldr.N_SEATS # 2
        ...
        self._refer_strategy_filler = StrategyFiller(tree=self, env_bldr=env_bldr)
    ...
    def build_tree(self):
        ...
        self.root = ChanceNode(env_state=self._env.state_dict(),
                               ...
                               depth=0)
        # self.root <PokerRL.game._.tree._.nodes.ChanceNode object at 0x7faa7c058978>
        self.root.allowed_actions = self._env.get_legal_actions() # [1,2]
        self.root.reach_probs = np.full(shape=(self._env_bldr.N_SEATS, self._env_bldr.rules.RANGE_SIZE),
                                        ...
                                        dtype=np.float32)
        self.root.sample_reach_probs = np.ones(shape=(self._env_bldr.N_SEATS, self._env_bldr.rules.RANGE_SIZE))

        # self.root.reach_probs.shape = (2, 6)
        # self._env_bldr.rules = <class 'PokerRL.game._.rl_env.game_rules.LeducRules'>
        
        # self.root.env_state = {'is_evaluating': True, 'current_round': 0, 
        # 'side_pots': [0, 0], 'main_pot': 2, 'board_2d': array([[-127, -127]], dtype=int8), 
        # 'last_action': [None, None, None], 'capped_raise': None, 'current_player': 0, 
        # 'last_raiser': None, 
        # 'deck_remaining': {'deck_remaining': array([[0, 0],[1, 0],[1, 1],[2, 0]], dtype=int8)},
        # 'n_actions_this_episode': 0, 'seats': [{'seat_id': 0, 'hand': array([[2, 1]], dtype=int8), 
        # 'hand_rank': None, 'stack': 12, 'current_bet': 0, 'is_allin': False, 
        # 'folded_this_episode': False, 'has_acted_this_round': False, 'side_pot_rank': -1}, 
        # {'seat_id': 1, 'hand': array([[0, 1]], dtype=int8), 'hand_rank': None, 
        # 'stack': 12, 'current_bet': 0, 'is_allin': False, 
        # 'folded_this_episode': False, 'has_acted_this_round': False, 
        # 'side_pot_rank': -1}], 'n_raises_this_round': 0}

        self._build_tree(current_node=self.root)

    def calcultate_variance(self):
        self._value_filler.calcultate_variance(self.root)

    def compute_ev(self):
        self._value_filler.compute_cf_values_heads_up(self.root)

    def compute_mc_ev(self, p_id):
        self._value_filler.compute_mc_cf_values_heads_up(node=self.root, p_id=p_id)
        # print("min p", m)

    def compute_vr_ev(self, p_id):
        self._value_filler.compute_vr_cf_values_heads_up(node=self.root, p_id=p_id)
        # print("min_p", m)
    ...
    def compute_reach_probs(self, p_id):
        self._strategy_filler.compute_reach_probs(p_id)
    ...
    def update_outer_stgy(self):
        self._value_filler.update_outer_stgy(node=self.root)

    def update_refer_info(self):
        self._value_filler.update_refer_info(node=self.root)

    def generate_samples(self, p_id, player, opponent, chance_p):
        self._strategy_filler.generate_samples(p_id, player, opponent, chance_p)

    def get_sample_strategy(self, node, his_id):
        # _sample_method in ['eps_greedy', 'vr']
        if self._sample_method == 'eps_greedy':
            return self._epsilon_greedy_sample(node, his_id)
        elif self._sample_method == 'vr':
            return self._vr_sample(node, his_id)
        else:
            raise ValueError
    ...
    def _epsilon_greedy_sample(self, node, his_id):
        stg = node.strategy[his_id] + self._epsilon
        return stg / np.sum(stg)

    def _vr_sample_copy(self, node, his_id):
        p_id = node.p_id_acting_next
        stg = node.strategy[his_id]
        ref_stg = node.ref_strategy[his_id]
        N_ACTIONS = len(stg)
        stg_diff = np.zeros(N_ACTIONS)
        for i in range(N_ACTIONS):
            stg_diff[i] = abs(ref_stg[i] -   stg[i]) + stg[i] * (node.V_value[p_id, his_id] + np.max(node.M_value[1-p_id]))
    ...
    def _vr_sample(self, node, his_id):
        p_id = node.p_id_acting_next
        stg = node.reach_probs[p_id, his_id] * node.strategy[his_id]
        ref_stg = node.ref_reach_probs[p_id, his_id] * node.ref_strategy[his_id]
        N_ACTIONS = len(stg)
        stg_diff = np.zeros(N_ACTIONS)
        for i in range(N_ACTIONS):
            stg_diff[i] = abs(ref_stg[i] - stg[i]) + stg[i] * (node.V_value[p_id, his_id] + np.max(node.M_value[1-p_id]))
        if np.sum(stg_diff) < 1e-5:
            p = self._epsilon_greedy_sample(node, his_id)
        else:
            p = stg_diff/np.sum(stg_diff)
        return p